...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("You have already signed up withe this email. Please login instead.")
...
def add_new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("get_all_posts"))
    return render_template("make-post.html", form=form, user=current_user, is_edit=False, logged_in=current_user.is_authenticated)
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", form=edit_form, user=current_user, is_edit=True, logged_in=current_user.is_authenticated)
